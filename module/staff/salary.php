...
$count=0;
$sql="SELECT *  FROM staff WHERE id='$check' ";
$st=$conn->query($sql);
$stinfo=mysqli_fetch_array($st);

$sql2 = "SELECT DISTINCT(date) FROM attendance WHERE attendedid='$check' and  MONTH( DATE ) = MONTH( CURRENT_DATE ) and YEAR( DATE )=YEAR( CURRENT_DATE )";
$resmon = $conn->query($sql2);
